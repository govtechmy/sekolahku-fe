...
try {
  ...
  const metaMap = {
    "/": {
      title: "Sekolahku - Home",
      description: "Selamat Datang ke Sekolahku",
      canonical: "/",
    },
    "/home": {
      title: "Sekolahku - Home",
    ...
    "/about": {
      title: "About Sekolahku",
      description: "Mengenai Sekolahku",
      canonical: "/about",
    },
    ...combined,
  };
  ...
  routes.forEach((route) => {
    ...
    html = html.replace(
      /<title[^>]*>.*<\/title>/,
      `<title data-rh="true">${escapeHTML(meta.title || "Sekolahku")}</title>`,
    );
    ...
    if (html.includes('<link rel="canonical"')) {
      html = html.replace(
        /<link rel="canonical"[^>]*>/,
        `<link rel="canonical" href="${escapeHTML(meta.canonical || route)}">`,
      );
    } else {
      // Inject before </head>
      html = html.replace(
  ...
  console.log(
    `✔ Full prerender completed with total of `,
    routes.length,
    `routes`,
  );
} catch (error) {
